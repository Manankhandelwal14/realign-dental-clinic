...
const Appointments: React.FC<AppointmentsProps> = ({ appointments, filters }) => {
    const { data: appointmentsData, current_page, links, per_page } = appointments;
        ...
        (appointment: Appointment) => {
            ...
            toast("Are you sure you want to delete this appointment?", {
                action: {
                    label: "Yes, Delete",
                    onClick: () => {
                        destroy(route("admin.appointments.destroy", appointment.id), {
                            onSuccess: () => toast.success("Appointment deleted successfully"),
                            onError: (errors) => {
                                toast.error(errors.message || "Failed to delete appointment");
                            },
                        });
                    },
                },
    ...
    const renderRow = (appointment: Appointment, index: number) => (
        <>
            ...
            <TableCell>{appointment.created_at ?? 'N/A'}</TableCell>
            <TableCell>
                <div className="flex gap-2">
                    <Button variant="outline" asChild>
                        <Link href={route("admin.appointments.show", appointment.id)}><Eye className="h-4 w-4" /></Link>
                    </Button>
                    <Button variant="outline" onClick={() => handleDelete(appointment)}><Trash className="h-4 w-4" /></Button>
                    <Button variant="outline" asChild>
                        <Link href={route("admin.appointments.edit", appointment.id)}><Pencil className="h-4 w-4" /></Link>
                    </Button>
                </div>
            </TableCell>
        </>
                    ...
                    <Table>
                        ...
                        <TableBody>
                            {appointmentsData.map((appointment, index) => (
                                <tr key={appointment.id}>{renderRow(appointment, index)}</tr>
                            ))}
